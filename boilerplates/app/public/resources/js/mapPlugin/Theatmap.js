...
(function(w){
 ...
 w.th337 = w.heatmapFactory = heatmapFactory;
})(window);
...
/*==============================以上部分为heatmap.js的核心代码,只负责热力图的展现====================================*/
 ...
 /**
  * @namespace BMap的所有library类均放在GMapLib命名空间下
  */
 var TMapLib = window.TMapLib = TMapLib || {};

 
 
 (function() {
     /** 
      * @exports HeatmapOverlay as TMapLib.HeatmapOverlay 
      */
     var HeatmapOverlay  = 
     /**
      * 热力图的覆盖物
      ...
      *
      * @example <b>参考示例：</b><br />
      * var map = new BMap.Map("container");<br />map.centerAndZoom(new BMap.Point(116.404, 39.915), 15);<br />var heatmapOverlay = new GMapLib.HeatmapOverlay({"radius":10, "visible":true, "opacity":70});<br />heatmapOverlay.setDataSet(data);//data是热力图的详细数据
      */
 
     TMapLib.HeatmapOverlay =  function(opts) {
         this.conf = opts;
         this.heatmap = null;
         this.latlngs = [];
         this.bounds = null;
    ...
    /**
     ...
     //移动时 根据地图位移同时位移
     HeatmapOverlay.prototype.changeDiv = function(top,left){
        if(this._div){
     ...
     HeatmapOverlay.prototype.initialize = function(map,id) {
         ...
         el.class = 'vtx_tmap_heatmap_class';
         el.className = 'vtx_tmap_heatmap_class';
         ...
         this.heatmap = th337.create(this.conf);
         this._div = el;
         return el;
     }
     ...
     HeatmapOverlay.prototype.draw = function() {
         ...
         var currentBounds = {
            xmin: this._map.getBounds().getSouthWest().lng,
            ymin: this._map.getBounds().getSouthWest().lat,
            xmax: this._map.getBounds().getNorthEast().lng,
            ymax: this._map.getBounds().getNorthEast().lat
         };
         if(!!this.bounds && 
            currentBounds.xmin == this.bounds.xmin &&
                currentBounds.ymin == this.bounds.ymin &&
                    currentBounds.xmax == this.bounds.xmax &&
                        currentBounds.ymax == this.bounds.ymax
            ){
             return;
         }
         this.bounds = currentBounds;
 
         var ne = this._map.lngLatToLayerPoint(new T.LngLat(currentBounds.xmax,currentBounds.ymax)),
             sw = this._map.lngLatToLayerPoint(new T.LngLat(currentBounds.xmin,currentBounds.ymin)),
             topY = ne.y,
             leftX = sw.x,
             h = sw.y - ne.y,
             w = ne.x - sw.x;
         ...
         if (this.data.data.length > 0) {
             ...
             while (len--) {
                 var latlng = this.data.data[len];
 
                 if (!(
                        latlng.lng >= currentBounds.xmin && latlng.lng <= currentBounds.xmax &&
                        latlng.lat >= currentBounds.ymin && latlng.lat <= currentBounds.ymax
                    )) {
                     continue;
                 }
 
                 var divPixel = this._map.lngLatToLayerPoint(new T.LngLat(latlng.lng,latlng.lat)),
                     screenPixel = {x: divPixel.x - leftX,y: divPixel.y - topY};
                 var roundedPoint = this.pixelTransform(screenPixel);
                 d.data.push({
                     x: roundedPoint.x,
                     y: roundedPoint.y,
                     count: this.data.data[len].count
                 });
             }
     ...
     HeatmapOverlay.prototype.setDataSet = function(data) {
         ...
         while (dlen--) {
             ...
             var divPixel = this._map.lngLatToLayerPoint(new T.LngLat(latlng.lng,latlng.lat)),
                 leftX = this._map.lngLatToLayerPoint(new T.LngLat(currentBounds.xmin,currentBounds.ymin)).x,
                 topY = this._map.lngLatToLayerPoint(new T.LngLat(currentBounds.xmax,currentBounds.ymax)).y,
                 screenPixel = {x:divPixel.x - leftX, y: divPixel.y - topY};
