...
(function(w){
 ...
 w.gish337 = w.heatmapFactory = heatmapFactory;
})(window);
...
/*==============================以上部分为heatmap.js的核心代码,只负责热力图的展现====================================*/
 ...
 (function() {
     /** 
      * @exports HeatmapOverlay as GMapLib.HeatmapOverlay 
      */
     var HeatmapOverlay  = 
     /**
      * 热力图的覆盖物
     ...
     GMapLib.HeatmapOverlay =  function(opts) {
         this.conf = opts;
         this.heatmap = null;
         this.latlngs = [];
         this.bounds = null;
    ...
    /**
     ...
     HeatmapOverlay.prototype.initialize = function(map,id) {
         ...
         el.style.left = 0;
         el.style.border = 0;
         el.style.width = this._map.width + "px";
         el.style.height = this._map.height + "px";
     ...
     HeatmapOverlay.prototype.draw = function() {
         ...
         var currentBounds = this._map.extent;
         if(!!this.bounds && 
         ...
         var ne = this._map.toScreen({x:currentBounds.xmax,y:currentBounds.ymax}),
             sw = this._map.toScreen({x:currentBounds.xmin,y:currentBounds.ymin}),
             topY = ne.y,
             leftX = sw.x,
             h = sw.y - ne.y,
             w = ne.x - sw.x;
         ...
         if (this.data.data.length > 0) {
             ...
             while (len--) {
                 ...
                 var divPixel = this._map.toScreen({x:latlng.lng,y:latlng.lat}),
     ...
     HeatmapOverlay.prototype.setDataSet = function(data) {
         ...
         while (dlen--) {
             ...
             var divPixel = this._map.toScreen({x:latlng.lng,y:latlng.lat}),
                 leftX = this._map.toScreen({x:currentBounds.xmin,y:currentBounds.ymin}).x,
                 topY = this._map.toScreen({x:currentBounds.xmax,y:currentBounds.ymax}).y,
                 screenPixel = {x:divPixel.x - leftX, y: divPixel.y - topY};
